...
type Model interface {
	...
	// SetBackgroundAgents updates the live background-agent roster shown in the
	// "Background agents (N)" section and the delegation breadcrumb's "+N" count.
	SetBackgroundAgents(tasks []agenttool.TaskInfo)
...
type model struct {
	...
	backgroundAgents   []agenttool.TaskInfo // Running background-agent tasks (sorted by start time)
...
// SetBackgroundAgents updates the live background-agent roster from a runtime
// snapshot. Only running tasks are kept (finished tasks linger in the snapshot)
// so the section drains as work completes, and they are sorted by start time so
// rows stay put across polls. It no-ops while the running set is and stays empty
// to avoid needless cache invalidation during ordinary turns.
func (m *model) SetBackgroundAgents(tasks []agenttool.TaskInfo) {
	var running []agenttool.TaskInfo
	for _, t := range tasks {
		if t.Status == agenttool.StatusRunning {
			running = append(running, t)
		}
	}
	slices.SortFunc(running, func(a, b agenttool.TaskInfo) int {
	...
	if len(running) == 0 && len(m.backgroundAgents) == 0 {
		return
	}
	m.backgroundAgents = running
	m.invalidateCache()
}
...
func (m *model) renderSections(contentWidth int) []string {
	...
	appendSection(m.backgroundAgentsSection(contentWidth))
...
//
// While background agents run concurrently, a muted "+N background" addendum is
// appended to hint at the off-chain work the Background agents panel lists in
// full. The chain is fitted against the width left after the addendum so the
// count is never clipped.
func (m *model) delegationBreadcrumb(contentWidth int) string {
	...
	suffix := ""
	if n := len(m.backgroundAgents); n > 0 {
		suffix = styles.MutedStyle.Render(fmt.Sprintf(" +%d background", n))
	}
	avail := contentWidth
	if avail > 0 {
		avail -= ansi.StringWidth(suffix)
	}
	...
	full := b.String()
	if avail <= 0 || ansi.StringWidth(full) <= avail {
		return full + suffix
	}
	...
	if ansi.StringWidth(elided) <= avail {
		return elided + suffix
	}
	return ansi.Truncate(full, avail, "…") + suffix
}
...
// backgroundAgentsSection renders the live "Background agents (N)" panel: one
// row per running background-agent task (run_background_agent), each with a
// colored activity dot and the agent name in its accent color, plus the elapsed
// run time right-aligned and muted. It returns "" when none are running so the
// panel surfaces only while concurrent background work is in flight and drains
// as tasks finish.
func (m *model) backgroundAgentsSection(contentWidth int) string {
	if len(m.backgroundAgents) == 0 {
		return ""
	}

	lines := make([]string, len(m.backgroundAgents))
	for i, t := range m.backgroundAgents {
		accent := styles.AgentAccentStyleFor(t.Agent)
		label := accent.Render("●") + " " + accent.Render(t.Agent)
		elapsed := styles.MutedStyle.Render(toolcommon.FormatDuration(time.Since(t.StartedAt)))
		gap := max(contentWidth-lipgloss.Width(label)-lipgloss.Width(elapsed), 1)
		lines[i] = label + strings.Repeat(" ", gap) + elapsed
	}

	title := fmt.Sprintf("Background agents (%d)", len(m.backgroundAgents))
	return m.renderTab(title, strings.Join(lines, "\n"), contentWidth)
}
